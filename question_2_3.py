...
alpha = 1
sum_j_spam = sum(teta_j_spam) + no_of_features * alpha
for j in range(len(teta_j_spam)):
    teta_j_spam[j] = (teta_j_spam[j] + alpha) / sum_j_spam

sum_j_ham = sum(teta_j_ham) + no_of_features * alpha
for j in range(len(teta_j_ham)):
    teta_j_ham[j] = (teta_j_ham[j] + alpha) / sum_j_ham
...
test_accuracy_laplace_file = open('test_accuracy_laplace.csv', mode='w', newline="")
test_accuracy_laplace_file.write(str(accuracy))    

test_accuracy_laplace_file.close()
labels_file.close()
